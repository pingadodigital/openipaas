...
async function ncmHandler(req: NextRequest, authContext: UnifiedAuthContext) {
  ...
  const path = `/produtos/ncm${searchParams.toString() ? `?${searchParams.toString()}` : ''}`
...
export const GET = withUnifiedAuth(ncmHandler)
