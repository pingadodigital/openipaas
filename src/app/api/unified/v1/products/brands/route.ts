...
import { mapContaAzulBrandToUnified } from '@/lib/mappers/products'

async function brandsHandler(req: NextRequest, authContext: UnifiedAuthContext) {
  ...
  const path = `/produtos/ecommerce-marcas${searchParams.toString() ? `?${searchParams.toString()}` : ''}`
  ...
  try {
    switch (linkedAccount.provider) {
      case 'CONTA_AZUL':
        ...
        const items = (data.items || []).map(mapContaAzulBrandToUnified)
...
export const GET = withUnifiedAuth(brandsHandler)
